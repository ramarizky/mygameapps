...
public struct HomeView: View {
    ...
    public var columns = Array(repeating: GridItem(.flexible()), count: 2)
    @Environment(\.managedObjectContext) var viewContext
    @StateObject var diViewModel: DIViewModel = DIViewModel()

    public init(homeVM: HomeViewModel) {
            ...
            if homeVM.homeState == .loaded {
                VStack {
                    HStack {
                        ...
                        Spacer()
                        NavigationLink(destination: FavouriteView(favVM: diViewModel.provideFavourite())) {
                    ...
                    ScrollView {
                        LazyVGrid(columns: columns, spacing: 16) {
                            ...
                            ForEach(homeVM.dataGames, id: \.id) { item in
                                NavigationLink(
                                    destination:
                                        DetailView(detailVM: diViewModel.provideDetail(), gameId: item.id ?? 0)
